> **Archived draft.** This is an earlier state of `requirements.md`, captured
> before the REQ-007 Monte Carlo uncertainty analysis and the REQ-008/REQ-009
> stability-margin analysis existed. Kept for the record, not for current
> traceability — see `requirements.md` for the current status.
